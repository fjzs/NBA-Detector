...
def show(image, target, image_aug, target_aug):    
    ...
    color_per_box_aug = [color_per_class[id.item()] for id in target_aug["labels"]]
    annotated_image = draw_bounding_boxes(image, target["boxes"], colors=color_per_box, width=3).permute(1, 2, 0).numpy()
    annotated_image_aug = draw_bounding_boxes(image_aug, target_aug["boxes"], colors=color_per_box_aug, width=3).permute(1, 2, 0).numpy()
...
if __name__ == "__main__":
    ...
    train_dataset_aug = BasketballDataset(root_dir=PATH_TO_DATASET, transform=transformation, image_set='train')
